[![pipeline status](https://gitlab.com/aruw/controls/qkrt-mcb/badges/develop/pipeline.svg)](https://gitlab.com/aruw/controls/qkrt-mcb/-/commits/develop)
...
```
cd my-amazing-project/qkrt-mcb-project
pipenv shell
# Build for hardware
scons build
# Run automated tests
...
1. Make sure you have VSCode opened in the folder `qkrt-mcb` (**not
   `qkrt-mcb-project`**)
...
1. Open the folder `aruw-qkrt-mcb-project` in VSCode. Hit the debug tab on the left side or type
...
`/qkrt-mcb-project/robot-type/robot_type.hpp` and change the macro defined in this file.
...
The below commands require that your working directory is `/qkrt-mcb-project` (where the
